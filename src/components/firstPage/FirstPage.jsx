...
import x from './img/x.png';
    ...
    export function FirstPage(props) {
        ...
        <form style={show00===false ? (true ? {display: "none"}: {display: "flex"} ) : {}} className={style.form} onSubmit={handleFormSubmit}>
            ...
            <lable className={style.formText}>  Vardą:<input className={style.formInput} onChange={e=>setName(e.target.value)} value={name} type="Jūsų vardas"/></lable> 
            <lable className={style.formText}>  Pavardę:<input className={style.formInput} onChange={e=>setSurname(e.target.value)} value={surname} type="Jūsų pavardė"/></lable> 
            <lable className={style.formText}>  Gimimo metus:<input className={style.formInput} onChange={e=>setBirthYear(e.target.value)} value={birthYear} type="Jūsų gimimo metai"/></lable> 
            <lable className={style.formText}>  Gimimo mėnesį:<input className={style.formInput} onChange={e=>setBirthMonth(e.target.value)} value={birthMonth} type="Jūsų gimimo mėnuo"/></lable> 
            <lable className={style.formText}>  Gimimo dieną:<input className={style.formInput} onChange={e=>setBirthDay(e.target.value)} value={birthDay} type="Jūsų gimimo diena"/></lable> 
            <lable className={style.formText}>  Elektroninį paštą:<input className={style.formInput} onChange={e=>setMail(e.target.value)} value={mail} type="Jūsų elektroninis paštas"/></lable> 
            <lable className={style.formText}>  Telefono numerį:<input className={style.formInput} onChange={e=>setPhone(e.target.value)} value={phone} type="Jūsų  telefono numeris"/></lable> 
            <lable className={style.formText}>  Slaptažodį:<input className={style.formInput} onChange={e=>setPass(e.target.value)} value={pass} type="Jūsų  slaptažodis"/></lable>
            <lable className={style.formText}>  Pakartokite slaptažodį:<input className={style.formInput} onChange={e=>setPassword(e.target.value)} value={password} type="Pakartokite slaptažodį"/></lable>  
